...
function git_plugin_redirect_after_action()
{
    wp_safe_redirect(git_plugin_get_page_url());
    exit;
}

function git_plugin_with_lock($path, $operation, callable $callback)
{
    $token = git_plugin_acquire_lock($path, $operation);
    ...
    try {
        ...
        git_plugin_release_lock($path, $token);
    }
}
...
function git_plugin_handle_connect_remote()
{
    ...
    git_plugin_with_lock($path, 'connect remote', function () use ($path, $remote_url) {
...
function git_plugin_handle_backup_current_branch()
{
    ...
    git_plugin_with_lock($path, 'backup branch', function () use ($path) {
        $current_branch = git_get_current_branch($path);
...
function git_plugin_handle_debug_repo()
{
    $path = git_get_valid_repo_path();
    ...
    git_plugin_with_lock($path, 'debug repository', function () use ($path) {
...
function git_plugin_handle_switch_branch()
{
    $path = git_get_valid_repo_path();
    ...
    git_plugin_with_lock($path, 'switch branch', function () use ($path, $branch) {
...
function git_plugin_handle_commit()
{
    $path = git_get_valid_repo_path();
    ...
    git_plugin_with_lock($path, 'commit', function () use ($path, $message) {
        ...
        $identity_result = git_plugin_ensure_repo_identity($path);

        if (!$identity_result['ok']) {
            git_plugin_finish_action('commit', 'failed', $identity_result['message'], 'error', $path);
            return;
        }

        if ($identity_result['configured']) {
            git_plugin_finish_action('commit', 'success', $identity_result['message'], 'updated', $path);
        }
...
function git_plugin_handle_pull()
{
    $path = git_get_valid_repo_path();
    ...
    git_plugin_with_lock($path, 'pull', function () use ($path) {
        if (!git_is_repo_clean($path)) {
        ...
        $result = run_git_command($path, 'pull --ff-only');
...
function git_plugin_handle_push()
{
    $path = git_get_valid_repo_path();
    ...
    git_plugin_with_lock($path, 'push', function () use ($path) {
...
function git_plugin_handle_create_branch()
{
    $path = git_get_valid_repo_path();
    ...
    git_plugin_with_lock($path, 'create branch', function () use ($path, $branch) {
        if (!git_is_repo_clean($path)) {
...
function git_plugin_handle_merge()
{
    $path = git_get_valid_repo_path();
    ...
    git_plugin_with_lock($path, 'merge branch', function () use ($path, $branch) {
        if (!git_branch_exists($path, $branch)) {
        ...
        if (!$identity_result['ok']) {
            git_plugin_finish_action('merge', 'failed', $identity_result['message'], 'error', $path);
            return;
        }

        if ($identity_result['configured']) {
            git_plugin_finish_action('merge', 'success', $identity_result['message'], 'updated', $path);
        }

        $result = run_git_command($path, 'merge --no-ff ' . escapeshellarg($branch));
...
function git_plugin_handle_delete_branch()
{
    $path = git_get_valid_repo_path();
    ...
    $delete_remote = !empty($_POST['delete_remote']);

    git_plugin_with_lock($path, 'delete branch', function () use ($path, $branch, $delete_remote) {
        if (!git_branch_exists($path, $branch)) {
        ...
        if ($result['status'] !== 0 || !$delete_remote) {
            return;
        }
...
function git_plugin_is_action_post()
{
...
add_action('admin_init', function () {
    if (!current_user_can('manage_options') || !git_plugin_is_action_post()) {
        return;
    }

    git_plugin_enable_notice_persistence();
    ...
    if (
        ...
        git_plugin_disable_notice_persistence();
        git_plugin_redirect_after_action();
    }
    ...
    if (
        ...
        git_plugin_handle_switch_branch();
        git_plugin_disable_notice_persistence();
        git_plugin_redirect_after_action();
    }
    ...
    if (
        ...
        git_plugin_handle_create_branch();
        git_plugin_disable_notice_persistence();
        git_plugin_redirect_after_action();
    }
    ...
    if (
        ...
        git_plugin_handle_delete_branch();
        git_plugin_disable_notice_persistence();
        git_plugin_redirect_after_action();
    }
